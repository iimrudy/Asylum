...
public class ProxyQueueRepository extends QueueRepository {
    ...
    public ProxyQueueRepository(ServerRepository repository) {
        ...
        // implement actionbar task that send the current queue status to the players that are in a queue
        Proxy.get().getServer().getScheduler().buildTask(Proxy.get(), () -> {
           for (var set : this.playerQueue.entrySet()) {
               Proxy.get().getServer().getPlayer(set.getKey()).ifPresent(player -> {
                   var cmp = serialize.apply("&e&lYou are in the &6" + set.getValue().name() + " &e&lqueue. &e&l(&6" + this.queuesSize.get(set.getValue()).get() + "&e&l).");
    ...
    public void onConnect(RedisQueueConnect queueConnect) {
        ...
        if (optionalServer.isPresent()) { // if server exist
            if (optionalQueue.isPresent()) { // if the player is in the limbo
                ...
                Proxy.get().getServer().getScheduler().buildTask(Proxy.get(), () -> optionalPlayer.get().createConnectionRequest(optionalServer.get()).fireAndForget()).schedule();
            }
        }
    }
    ...
    public void onJoinQueue(RedisQueueJoin queueJoin) {
        Proxy.get().getServer().getPlayer(queueJoin.getPlayerName()).ifPresent(player -> { // if player exist
            // add task to the map
            this.playerQueue.put(queueJoin.getPlayerName(), queueJoin.getServerType());

            // update the queue size
            Proxy.get().getQueueRepository().getRepository().getAsylumDB().getRedisConnection().async().scard(queueJoin.getServerType().getQueueName()).thenAccept(
                    size -> ProxyQueueRepository.this.queuesSize.get(queueJoin.getServerType()).set(size) // update queue size
            );
        });
    }
    ...
    public void onQueueLeft(RedisQueueLeft queueLeft) {
        ...
        if (oldQueue != null) {
            ...
            Proxy.get().getQueueRepository().getRepository().getAsylumDB().getRedisConnection().async().scard(oldQueue.getQueueName()).thenAccept(
                    size -> ProxyQueueRepository.this.queuesSize.get(finalOldQueue).set(size) // update queue size
            );
        }
    }
}
