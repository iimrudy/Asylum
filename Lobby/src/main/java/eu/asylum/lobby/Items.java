...
public class Items {
    ...
    private static final ItemStack sword = new ItemStack(Material.IRON_SWORD, 1);
    private static final ItemStack clock = new ItemStack(Material.CLOCK, 1);

    static {
        var compassItemMeta = compass.getItemMeta();
        compassItemMeta.displayName(LegacyComponentSerializer.legacyAmpersand().deserialize("&aQuick Selector"));
        compass.setItemMeta(compassItemMeta);


        var chestItemMeta = chest.getItemMeta();
        chestItemMeta.displayName(LegacyComponentSerializer.legacyAmpersand().deserialize("&eCosmetics Menu"));
        chest.setItemMeta(chestItemMeta);


        var swordItemMeta = sword.getItemMeta();
        swordItemMeta.setUnbreakable(true);
        swordItemMeta.displayName(LegacyComponentSerializer.legacyAmpersand().deserialize("&aPVP Enabler"));
