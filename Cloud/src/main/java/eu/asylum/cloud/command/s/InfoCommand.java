    ...
    public void onCommand(String[] args) {
        ...
        if (args.length == 0) {
            ...
            for (ServerType s : ServerType.values()) {
                builder.append("\n").append(s.name()).append(" | ").append(Cloud.getInstance().getRepository().getOnlinePlayers()).append(" | ").append(Cloud.getInstance().getRepository().getServerCount(s));
            }
        } else {
            ...
            if (s != null) {
                ...
                builder.append("\n\nServer Count (").append(Cloud.getInstance().getRepository().getServerCount(s)).append(").");
            } else {
