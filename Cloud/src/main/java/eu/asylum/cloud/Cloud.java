...
public class Cloud {
  ...
  private Optional<Server> hostServer0(Server server) {
    ...
    new SyncConsoleCommand(
        ...
        logs -> {
          ...
          asylumDB.publishJson(
              CloudChannels.SERVER_ADD.getChannel(),
              new RedisCloudAdd(server)); // announcing that the server is started
          asylumDB
              .getMongoCollection("asylum", "cloud")
  ...
  public Optional<Server> hostServer(ServerType type) {
    ...
    if (pathTo.exists()) { // if the server-folder already exists
      ...
      } else { // server is not persistent, cloud had some problem while removing files (?) -
